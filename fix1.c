 ...
 */
int var_environ(op_t *obs, var_t **h, char *offset)
{
	int i, j, k;

	for (i = 0; obs->environ[i]; i++)
	{
		for (j = 0, k = 1; obs->environ[i][j]; j++)
		{
			if (obs->environ[i][j] == '=')
			{
				*h = add_varnode(h, obs->environ[i] + j + 1, k);
				return (k - 1);
			}
			if (obs->environ[i][j] != offset[k])
				break;
			k++;
		}
	}
 ...
 */
int var_builtin(op_t *obs, var_t **h, char *offset, char *exitcode)
{
	switch (offset[1])
	{
	case '?':
 ...
 */
int link_variables(op_t *obs, var_t **h, char *input, char *exitcode)
{
	unsigned int i, len;
	int flag = -1, count = 0;
	var_t *c;
	...
	for (i = 0; input[i]; i++)
	{
		if (input[i] == '$')
		{
			flag = var_builtin(obs, h, input + i, exitcode);
			if (flag == -1)
				flag = var_environ(obs, h, input + i);
			i += flag;
		}
	}
...
char *exp_variables(op_t *obs, char *s)
{
	...
	h = NULL;
	exitcode = _itoa(obs->exCo);
	len = link_variables(obs, &h, s, exitcode);
